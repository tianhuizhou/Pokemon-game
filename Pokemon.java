...
public abstract class Pokemon implements Comparable<Pokemon> {
  ...
  * @throws PokemonException if no name, the exception will be thrown.
  */
   public void setName(String newName)throws PokemonException {
      if (newName.length() > 0) {
         ...
         pe.setMessage("The name should not be empty or contain only space");
         throw pe;
      }
   }
   ...
   */  
   ...
   /**
   * method overriden.
   * compare the two object and return a number.
   * @return negative number if p1 is less than p2.
   * positive number if p1 is greater than p2.
   * @param p pokemon p will be compared.
   */
   public int compareTo(Pokemon p) {
      /* varibles for pokemon numbers.*/
      int num1 = this.number;
      int num2 = p.getNumber();
      /* varibles for pokemon names.*/
      String name1 = this.name;
      String name2 = p.getName();
      /* varibles for pokemon hp.*/
      int hP1 = this.hP;
      int hP2 = p.getHP();
      /* varibles for pokemon cp.*/
      int cP1 = this.cP;
      int cP2 = p.getCP();
      /* varibles for pokemon fastAttack.*/
      String fastAttack1 = this.fastAttack;
      String fastAttack2 = p.getFastAttack();
      /* varibles for pokemon specialAttack.*/
      String specialAttack1 = this.specialAttack;
      String specialAttack2 = p.getSpecialAttack();
      ...
      if (num1 != num2) {
         ...
         r = name1.toUpperCase().compareTo(name2.toUpperCase());
         if (r == 0) {
            if (hP1 != hP2) {
               r = hP1 - hP2;
            } else {
               if (cP1 != cP2) {
                  ...
                  r = fastAttack1.compareTo(fastAttack2);
                  if (r == 0) {
                     r = specialAttack1.compareTo(specialAttack2);
                  }
               }
            }
         }
   ...
   /**
    * method overriden.
    * @return true if two pokemon are same.
    *  false if two pokemon are different.
    * @param p another pokemon object.
    */
